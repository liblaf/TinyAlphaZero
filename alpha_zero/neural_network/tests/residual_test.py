...
from ..residual import NeuralNetworkWrapper as NetworkWrapper
...
def test_network_wrapper_checkpoint(board_size: int = 5) -> None:
    ...
    try:
        ...
        net.checkpoint_save(filepath=checkpoint_filepath)
        assert checkpoint_filepath.exists()
        net.checkpoint_load(filepath=checkpoint_filepath)
    finally:
